...
export default function RegistroIndividual() {
  ...
  return (
    <div>
      ...
      {/* ====== NOTA PEDAGÓGICA (RAW) ====== */}
      <div className="card" style={{ padding: "20px 24px", marginBottom: 24, border: "2px solid #E2E8F0" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", cursor: "pointer" }} onClick={() => setIsPedagogicoOpen(!isPedagogicoOpen)}>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            <div style={{ width: 40, height: 40, borderRadius: 12, background: "#F1F5F9", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20 }}>
              📝
            </div>
            <div>
              <p style={{ margin: 0, fontSize: 15, fontWeight: 800, color: "#1E293B" }}>Anotação Pedagógica</p>
              <p style={{ margin: 0, fontSize: 12, color: "#64748B" }}>Relato livre para o relatório final</p>
            </div>
          </div>
        ...
        {isPedagogicoOpen && (
          <div style={{ marginTop: 20, paddingTop: 20, borderTop: "1px solid #E2E8F0" }}>
            <p style={{ margin: "0 0 12px", fontSize: 13, color: "#334155" }}>
              O que você observou de importante no {student?.nome || "aluno"} hoje?
            </p>
            <div style={{ position: "relative" }}>
              <textarea
                value={iaText}
                onChange={e => setIaText(e.target.value)}
                placeholder="Ex: Conseguiu montar o quebra-cabeça sozinho..."
            ...
            <button 
              onClick={async () => {
                ...
                try {
                  const payload = {
                    ...
                    professorId: profile.uid,
                    pilar: "geral",
                    pilarLabel: "Observação Geral",
                    nota: iaText,
                    sentimento: "neutro"
                  };
            
                  await fetch("/api/pedagogico", {
                    method: "POST",
              ...
              disabled={!iaText.trim() || iaState === "saving"} 
              style={{ width: "100%", marginTop: 12, padding: "14px", borderRadius: 12, border: "none", background: iaText.trim() ? "#F97316" : "#E2E8F0", color: iaText.trim() ? "white" : "#94A3B8", fontWeight: 700, fontSize: 14, cursor: iaText.trim() ? "pointer" : "not-allowed", transition: "0.2s" }}
            >
              {iaState === "saving" ? "Enviando..." : "Enviar nota"}
            </button>
          </div>
        )}
      </div>
