...
export default function AdminMatriculasPage() {
  ...
  const handleSelectSolic = (solic: any) => {
  ...
  const handleStartEdit = () => {
    setEditForm(JSON.parse(JSON.stringify(selectedSolic)));
    setIsEditing(true);
  };
  ...
  const handleEditField = (section: string, field: string, value: any) => {
  ...
  const handleSaveEdit = async () => {
    if (!editForm.aluno.nome.trim()) {
      alert("Por favor, preencha o nome do aluno.");
      return;
    }
    setProcessing(true);
    try {
      await updateMatriculaSolicitude(editForm.id, editForm);
      setSelectedSolic(editForm);
      setIsEditing(false);
      setEditForm(null);
      ...
      console.error(err);
      alert("Erro ao salvar alterações da ficha.");
    } finally {
      setProcessing(false);
    }
  };
  ...
  return (
    <div>
      <header className="no-print" style={{ marginBottom: 32, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div>
      ...
      {/* Tabs */}
      <div className="no-print" style={{ display: "flex", gap: 16, borderBottom: "1px solid #E2E8F0", marginBottom: 24 }}>
        {(["pendente", "aprovada", "recusada"] as const).map(tab => {
          const count = solicitudes.filter(s => s.status === tab).length;
          return (
            <button
              key={tab}
              onClick={() => { setActiveTab(tab); setSelectedSolic(null); setIsEditing(false); }}
              style={{
                padding: "12px 16px",
                background: "none",
                border: "none",
      ...
      <div style={{ display: "grid", gridTemplateColumns: "320px 1fr", gap: 24 }}>
        {/* Left List */}
        <div className="card no-print" style={{ padding: 0, overflow: "hidden", display: "flex", flexDirection: "column" }}>
            ...
            {filtered.length === 0 ? (
              ...
              filtered.map(solic => {
                ...
                return (
                  <div 
                    key={solic.id}
                    onClick={() => handleSelectSolic(solic)}
                    style={{
                      padding: "16px 20px",
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", borderBottom: "1px solid #E2E8F0", paddingBottom: 16 }}>
                <div>
                  <h2 style={{ margin: 0, fontSize: 18, fontWeight: 800, color: "#1E293B" }}>
                    {isEditing ? `Editando Ficha de ${editForm?.aluno?.nome}` : `Ficha de ${selectedSolic.aluno?.nome}`}
                  </h2>
                  <span style={{ fontSize: 11, background: "#E0F2FE", color: "#0369A1", fontWeight: 700, padding: "2px 8px", borderRadius: 6, display: "inline-block", marginTop: 6 }}>
                    Turma: {isEditing ? editForm?.aluno?.turma : selectedSolic.aluno?.turma}
                  </span>
                </div>
                  ...
                  {isEditing ? (
                    <>
                      ...
                      <button 
                        ...
                        style={{ fontSize: 13, padding: "8px 16px" }}
                      >
                        {processing ? "Salvando..." : "Salvar Ficha"}
                      </button>
                    </>
                  ) : (
                    <>
              ...
              {!isEditing ? (
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24, fontSize: 12 }}>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                    <h4 style={{ margin: 0, fontSize: 14, color: "var(--primary-dark)", fontWeight: 800 }}>EDITAR DADOS DO ALUNO</h4>
                    <div>
                      <label style={{ display: "block", fontWeight: 700, marginBottom: 4 }}>Nome do Aluno</label>
                      <input type="text" className="text-input" value={editForm.aluno.nome} onChange={e => handleEditField('aluno', 'nome', e.target.value)} />
                    </div>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                      <div>
                        <label style={{ display: "block", fontWeight: 700, marginBottom: 4 }}>Nascimento</label>
                        <input type="date" className="text-input" value={editForm.aluno.dataNascimento} onChange={e => handleEditField('aluno', 'dataNascimento', e.target.value)} />
                      </div>
                      <div>
                        <label style={{ display: "block", fontWeight: 700, marginBottom: 4 }}>Turma</label>
                        <select className="text-input" value={editForm.aluno.turma} onChange={e => handleEditField('aluno', 'turma', e.target.value)}>
                          {TURMAS_SUGERIDAS.map(t => <option key={t} value={t}>{t}</option>)}
                        </select>
                      </div>
                    </div>

                    <h4 style={{ margin: "10px 0 0", fontSize: 14, color: "var(--primary-dark)", fontWeight: 800 }}>EDITAR DADOS DA MÃE</h4>
                    <div>
                      <label style={{ display: "block", fontWeight: 700, marginBottom: 4 }}>Nome da Mãe</label>
                      <input type="text" className="text-input" value={editForm.mae.nome} onChange={e => handleEditField('mae', 'nome', e.target.value)} />
                    </div>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                      <div>
                        <label style={{ display: "block", fontWeight: 700, marginBottom: 4 }}>CPF (Apenas números)</label>
                        <input type="text" className="text-input" value={editForm.mae.cpf} onChange={e => handleEditField('mae', 'cpf', e.target.value)} />
                      </div>
                      <div>
                        <label style={{ display: "block", fontWeight: 700, marginBottom: 4 }}>RG</label>
                        <input type="text" className="text-input" value={editForm.mae.rg} onChange={e => handleEditField('mae', 'rg', e.target.value)} />
                      </div>
                    </div>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                      <div>
                        <label style={{ display: "block", fontWeight: 700, marginBottom: 4 }}>E-mail</label>
                        <input type="email" className="text-input" value={editForm.mae.email} onChange={e => handleEditField('mae', 'email', e.target.value)} />
                      </div>
                      <div>
                        <label style={{ display: "block", fontWeight: 700, marginBottom: 4 }}>Celular</label>
                        <input type="text" className="text-input" value={editForm.mae.telCelular} onChange={e => handleEditField('mae', 'telCelular', e.target.value)} />
                      </div>
                    </div>
                  </div>
                  ...
                  {/* Col 2 */}
                  <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                    <h4 style={{ margin: 0, fontSize: 14, color: "var(--primary-dark)", fontWeight: 800 }}>EDITAR ENDEREÇO & SAÚDE</h4>
                    <div>
                      <label style={{ display: "block", fontWeight: 700, marginBottom: 4 }}>Rua, Nº, Compl.</label>
                      <input type="text" className="text-input" value={editForm.endereco.rua} onChange={e => handleEditField('endereco', 'rua', e.target.value)} />
                    </div>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                      <div>
                        <label style={{ display: "block", fontWeight: 700, marginBottom: 4 }}>CEP</label>
                        <input type="text" className="text-input" value={editForm.endereco.cep} onChange={e => handleEditField('endereco', 'cep', e.target.value)} />
                      </div>
                      <div>
                        <label style={{ display: "block", fontWeight: 700, marginBottom: 4 }}>Tel. Residencial</label>
                        <input type="text" className="text-input" value={editForm.endereco.telResidencial} onChange={e => handleEditField('endereco', 'telResidencial', e.target.value)} />
                      </div>
                    </div>
                    <div>
                      <label style={{ display: "block", fontWeight: 700, marginBottom: 4 }}>Alergias</label>
                      <input type="text" className="text-input" value={editForm.saude.alergias} onChange={e => handleEditField('saude', 'alergias', e.target.value)} />
                    </div>

                    <h4 style={{ margin: "10px 0 0", fontSize: 14, color: "var(--primary-dark)", fontWeight: 800 }}>EDITAR DADOS DO PAI</h4>
                    <div>
                      <label style={{ display: "block", fontWeight: 700, marginBottom: 4 }}>Nome do Pai</label>
                      <input type="text" className="text-input" value={editForm.pai.nome} onChange={e => handleEditField('pai', 'nome', e.target.value)} />
                    </div>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                      <div>
                        <label style={{ display: "block", fontWeight: 700, marginBottom: 4 }}>CPF (Apenas números)</label>
                        <input type="text" className="text-input" value={editForm.pai.cpf} onChange={e => handleEditField('pai', 'cpf', e.target.value)} />
                      </div>
                      <div>
                        <label style={{ display: "block", fontWeight: 700, marginBottom: 4 }}>RG</label>
                        <input type="text" className="text-input" value={editForm.pai.rg} onChange={e => handleEditField('pai', 'rg', e.target.value)} />
                      </div>
                    </div>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                      <div>
                        <label style={{ display: "block", fontWeight: 700, marginBottom: 4 }}>E-mail</label>
                        <input type="email" className="text-input" value={editForm.pai.email} onChange={e => handleEditField('pai', 'email', e.target.value)} />
                      </div>
                      <div>
                        <label style={{ display: "block", fontWeight: 700, marginBottom: 4 }}>Celular</label>
                        <input type="text" className="text-input" value={editForm.pai.telCelular} onChange={e => handleEditField('pai', 'telCelular', e.target.value)} />
                      </div>
                    </div>
                  </div>
                </div>
